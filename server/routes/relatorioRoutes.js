...
const { RegistrosPonto, Funcionarios, Feriados } = require('../repository');
...
async function relatorioDoFuncionario(funcionario, inicio, fim, hojeKey, feriados) {
  const { inicioUtcIso, fimUtcIso } = utcBoundsForLocalRange(inicio, fim);
  const [registros, feriadosSet] = await Promise.all([
    RegistrosPonto.doFuncionarioEntrePeriodo(funcionario.id, inicioUtcIso, fimUtcIso),
    feriados ? Promise.resolve(feriados) : Feriados.listarComoSet(),
  ]);
  return calcularRelatorio(funcionario, registros, inicio, fim, hojeKey, feriadosSet);
}
...
async function linhasEquipe(funcionarios, inicio, fim, hojeKey) {
  // Busca o calendário de feriados uma única vez e reaproveita pra todos os
  // colaboradores, em vez de consultar o banco de novo pra cada um.
  const feriados = await Feriados.listarComoSet();
  return Promise.all(
    funcionarios.map(async (f) => ({
      funcionario: Funcionarios.publico(f),
      totais: (await relatorioDoFuncionario(f, inicio, fim, hojeKey, feriados)).totais,
    }))
  );
}

function register(router) {
  // ---------- Relatório do próprio colaborador ----------
  router.get(
    '/api/relatorio/meu',
  ...
  // ---------- Relatório consolidado da equipe (somente admin) ----------
  router.get(
