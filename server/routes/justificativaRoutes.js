...
function register(router) {
  // ---------- Colaborador: suas próprias justificativas ----------
  router.get(
    '/api/justificativas',
  ...
  router.post(
    ...
    withAuth(async ({ req, res, user }) => {
      ...
      if (descricao.length > 2000) {
        return sendJson(res, 400, { erro: 'Descrição muito longa (máximo 2000 caracteres).' });
      }
  ...
  // Usado para montar a folha de impressão (o próprio colaborador ou um admin podem ver).
  router.get(
    '/api/justificativas/:id',
    withAuth(async ({ res, params, user }) => {
      const id = parseInt(params.id, 10);
      const justificativa = await Justificativas.porId(id);
      if (!justificativa) return sendJson(res, 404, { erro: 'Justificativa não encontrada.' });
      if (justificativa.funcionario_id !== user.id && !user.is_admin) {
        return sendJson(res, 403, { erro: 'Sem permissão para ver esta justificativa.' });
      }
  ...
  router.delete(
    ...
    withAuth(async ({ res, params, user }) => {
      ...
      if (!justificativa) return sendJson(res, 404, { erro: 'Justificativa não encontrada.' });
      if (justificativa.funcionario_id !== user.id && !user.is_admin) {
        return sendJson(res, 403, { erro: 'Sem permissão para remover esta justificativa.' });
      }
  ...
  // ---------- Administração: acompanhar justificativas de um colaborador ----------
  router.get(
    '/api/funcionarios/:id/justificativas',
    withAdmin(async ({ res, params }) => {
      const id = parseInt(params.id, 10);
      const alvo = await Funcionarios.porId(id);
      if (!alvo) return sendJson(res, 404, { erro: 'Colaborador não encontrado.' });
