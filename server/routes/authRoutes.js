...
function register(router) {
  router.post('/api/login', async (req, res) => {
    ...
    // não quebrar quem ainda tem o e-mail salvo/memorizado.
    ...
    if (!user || !user.ativo || !verifyPassword(senha, user.senha_hash)) {
      return sendJson(res, 401, { erro: 'Login ou senha inválidos.' });
    }
  ...
  router.put(
    ...
    withAuth(async ({ req, res, user }) => {
      ...
      if (!nome || nome.length < 2) return sendJson(res, 400, { erro: 'Informe um nome válido.' });
      if (!isValidEmail(email)) return sendJson(res, 400, { erro: 'Informe um e-mail válido.' });
      const existente = await Funcionarios.porEmail(email);
      if (existente && existente.id !== user.id) {
        return sendJson(res, 409, { erro: 'Já existe um usuário com esse e-mail.' });
      }
