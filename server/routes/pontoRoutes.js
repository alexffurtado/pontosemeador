    ...
    withAuth(async ({ res, user }) => {
      ...
      sendJson(res, 200, {
        registros: doDiaAtual.map((r) => ({
          ...
          dataHoraUtc: new Date(r.data_hora_utc).toISOString(),
        })),
        proximoTipo: proximo,
        ...
        // Estado atual (independente do dia) — usado pela tela pra mostrar um
        // selo "em expediente" com contador ao vivo de tempo trabalhado.
        ultimoTipo: ultimoGeral ? ultimoGeral.tipo : null,
        ultimoDataHoraUtc: ultimoGeral ? new Date(ultimoGeral.data_hora_utc).toISOString() : null,
      });
    })
  );
