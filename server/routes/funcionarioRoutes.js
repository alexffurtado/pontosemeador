...
function validarDadosFuncionario(body, { exigirSenha }) {
  ...
  const tipoEscalaBruto = isValidTipoEscala(body.tipo_escala) ? body.tipo_escala : 'semanal';
  let escala_data_referencia = null;
  if (tipoEscalaBruto === '12x36') {
    escala_data_referencia = body.escala_data_referencia || '';
    if (!isValidDateKey(escala_data_referencia)) {
      erros.push('Informe uma data de referencia valida para a escala 12x36.');
    }
  }
  // Horario personalizado por dia da semana so faz sentido na escala "semanal"
  // (12x36 ja define o dia de trabalho por paridade de data, nao por dia da semana).
  const horarioPersonalizadoSemana = tipoEscalaBruto === 'semanal' && !!body.horario_personalizado_semana;
  let horariosSemana = null;
  if (horarioPersonalizadoSemana) {
    if (!isValidHorariosSemana(body.horarios_semana)) {
      erros.push('Horarios personalizados por dia da semana invalidos. Preencha entrada, saida e carga horaria para cada dia marcado como "trabalha".');
    } else {
      horariosSemana = body.horarios_semana.map((item) => ({
        dia: item.dia,
        ativo: !!item.ativo,
        entrada: item.ativo ? item.entrada : null,
        saida: item.ativo ? item.saida : null,
        carga_minutos: item.ativo ? Math.round(Number(item.carga_minutos)) : 0,
      }));
    }
  }
  ...
  const dados = {
    ...
    tipo_escala: tipoEscalaBruto,
    escala_data_referencia: tipoEscalaBruto === '12x36' ? escala_data_referencia : null,
    horario_personalizado_semana: horarioPersonalizadoSemana,
    horarios_semana: horarioPersonalizadoSemana ? horariosSemana : null,
  };
