...
const Feriados = {
  ...
  // Retorna um Set com as datas (YYYY-MM-DD) de todos os feriados cadastrados
  // — formato pronto pra ser passado a calcularRelatorio().
  async listarComoSet() {
    const { rows } = await pool.query('SELECT data FROM feriados');
    return new Set(rows.map((r) => r.data));
  },
  async criar({ data, descricao }) {
    const { rows } = await pool.query(
  ...
  async porData(data) {
    const { rows } = await pool.query('SELECT * FROM feriados WHERE data = $1', [data]);
    return rows[0] || null;
  },
  async remover(id) {
    const { rowCount } = await pool.query('DELETE FROM feriados WHERE id = $1', [id]);
    return rowCount > 0;
  },
};

module.exports = { Funcionarios, RegistrosPonto, Justificativas, Feriados };
