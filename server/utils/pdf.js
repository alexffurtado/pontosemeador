 ...
 * @param {Array<{titulo:string, subtitulo?:string}>} [opts.assinaturas] campos de
 *   assinatura desenhados lado a lado ao final do documento (ex.: colaborador e
 *   gestor), usados pra validar/aprovar o pagamento de horas extras impresso.
 * @param {boolean} [opts.landscape]
 */
function gerarRelatorioPdf({ titulo, subtitulo, colunas, linhas, resumo = [], assinaturas = [], landscape = false }) {
  ...
  // Campos de assinatura (ex.: colaborador + gestor), desenhados lado a lado
  // — pra imprimir o relatório e formalizar a aprovação do pagamento das
  // horas extras. Fica sempre junto num mesmo bloco (nunca quebra entre
  // colaborador e gestor em páginas diferentes).
  if (assinaturas.length) {
    const alturaBloco = 78;
    pdf.ensureSpace(alturaBloco, () => desenharCabecalho());
    pdf.y -= 36;
    const larguraColuna = usableWidth / assinaturas.length;
    const larguraLinha = Math.min(230, larguraColuna - 30);
    for (let i = 0; i < assinaturas.length; i++) {
      const assinatura = assinaturas[i];
      const centroX = margin + larguraColuna * i + larguraColuna / 2;
      const inicioX = centroX - larguraLinha / 2;
      const fimX = centroX + larguraLinha / 2;
      pdf.line(inicioX, pdf.y, fimX, pdf.y, PRETO, 0.8);
      const tituloTexto = assinatura.titulo || '';
      const tituloLargura = approxTextWidth(tituloTexto, 9.5, true);
      pdf.text(centroX - tituloLargura / 2, pdf.y - 14, tituloTexto, { size: 9.5, bold: true, color: VERDE_ESCURO });
      if (assinatura.subtitulo) {
        const subLargura = approxTextWidth(assinatura.subtitulo, 8, false);
        pdf.text(centroX - subLargura / 2, pdf.y - 27, assinatura.subtitulo, { size: 8, color: [0.4, 0.4, 0.4] });
      }
    }
    pdf.y -= 40;
  }
