...
// Toques feitos a poucos minutos um do outro geralmente são o colaborador
// corrigindo na hora um toque errado (ex.: apertou "saída (intervalo)" por
// engano e, um minuto depois, bateu "saída" de verdade) — nesses casos não
// faz sentido marcar o dia como "incompleto" por causa da marcação redundante.
const LIMIAR_CORRECAO_MS = 5 * 60 * 1000; // 5 minutos
...
function montarSessoes(registrosOrdenados, hojeKey) {
  const sessoes = []; // { inicio, fim, minutos, diaInicio, fimId }
  const diasInconsistentes = new Set();
  let aberto = null;
  let ultimoEvento = null; // evento anterior (qualquer tipo), so pra detectar toque duplicado por engano
  ...
  for (const ev of registrosOrdenados) {
    ...
    const provavelCorrecao = !!ultimoEvento && ev.dt - ultimoEvento.dt <= LIMIAR_CORRECAO_MS;
    if (TIPO_ENTRADA_EVENTOS.has(ev.tipo)) {
      if (aberto && !provavelCorrecao) {
      ...
      if (!aberto) {
        ...
        sessoes.push({ inicio: aberto, fim: ev.dt, minutos: minutesDiff(aberto, ev.dt), diaInicio, fimId: ev.id });
        aberto = null;
      }
    }
    ultimoEvento = ev;
 ...
 * @param {Set<string>} [feriados] datas (YYYY-MM-DD) de feriados — calendário
 *   único, vale pra qualquer colaborador. Num feriado a meta de horas vira
 *   zero (como um dia de folga) e tudo que for trabalhado conta como extra,
 *   mesmo que o dia caia dentro do expediente normal ou da escala 12x36.
 */
function calcularRelatorio(funcionario, registros, inicioKey, fimKey, hojeKey, feriados = new Set()) {
  ...
  // Pra cada marcação que FECHA uma sessão, guarda em que dia essa sessão
  // COMEÇOU. Usado abaixo pra "saída antecipada" ignorar o fechamento de um
  // plantão/sobreaviso que começou de madrugada num dia diferente — comparar
  // esse horário com o fim do expediente normal não faz sentido.
  const diaInicioPorFimId = new Map();
  for (const s of sessoes) {
    if (s.fimId != null) diaInicioPorFimId.set(s.fimId, s.diaInicio);
  }
  ...
  for (const dataKey of dateKeyRange(inicioKey, fimKey)) {
    ...
    const configBase = configDoDia(dataKey, weekday);
    const { entradaEsperadaMin, saidaEsperadaMin, cargaEsperadaMin } = configBase;
    // Feriado sobrescreve qualquer escala: o dia deixa de ter meta de horas
    // (igual uma folga), mesmo que fosse dia de trabalho normal ou dia de
    // plantão na escala 12x36.
    const feriado = feriados.has(dataKey);
    const ehDiaUtil = configBase.ehDiaUtil && !feriado;
    ...
    // So considera, pra fins de "saída antecipada", uma saída que fecha uma
    // sessão iniciada NESSE MESMO DIA. Isso evita comparar o fechamento de um
    // plantão/sobreaviso que começou de madrugada no dia anterior (ou que só
    // termina de madrugada no dia seguinte) com o horário normal de saída do
    // expediente — que é o que gerava avisos de "saída antecipada" de
    // 15h/16h em dias com plantão, mesmo com as horas certas.
    const ultimaSaida = [...eventosDoDia]
      .reverse()
      .find((e) => e.tipo === 'saida' && !(diaInicioPorFimId.has(e.id) && diaInicioPorFimId.get(e.id) !== dataKey));

    let atrasoMin = 0;
    // Atraso/saída antecipada só fazem sentido em dia de trabalho — num dia de
    // folga não existe um horário esperado pra comparar.
    if (ehDiaUtil && verificarAtraso && primeiraEntrada) {
    ...
    if (ehDiaUtil && verificarSaidaAntecipada && ultimaSaida) {
