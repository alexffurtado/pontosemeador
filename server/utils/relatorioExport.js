...
function statusDoDia(dia) {
  ...
  if (dia.eventos.length === 0) {
    ...
    return dia.diaUtil ? '-' : 'Folga';
  }
  const partes = [];
  if (dia.feriado) partes.push('Feriado');
...
function pdfRelatorioDetalhado(funcionario, dias, totais, periodoLabel) {
  ...
  return gerarRelatorioPdf({
    ...
    linhas,
    resumo,
    // Campos de assinatura pra formalizar, na folha impressa, a aprovação do
    // pagamento das horas extras apuradas acima.
    assinaturas: [
      { titulo: 'Assinatura do colaborador', subtitulo: funcionario.nome },
      { titulo: 'Assinatura do gestor / responsável', subtitulo: 'Aprovação para pagamento das horas extras' },
    ],
  });
}
