...
require('./server/routes/justificativaRoutes').register(router);
require('./server/routes/feriadoRoutes').register(router);
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (!served) {
      ...
      res.end('<h1>404 - Página não encontrada</h1><p><a href="/login.html">Voltar ao login</a></p>');
    }
  } catch (err) {
...
// A inicialização do banco (criação de tabelas/migrações/seed do admin) é
// assíncrona com Postgres, então só começamos a aceitar conexões depois que
...
async function start() {
  try {
    ...
    console.error('Verifique se a variável de ambiente DATABASE_URL está');
    console.error('correta e se o banco está acessível.');
    console.error('----------------------------------------------------');
    process.exit(1);
  }
