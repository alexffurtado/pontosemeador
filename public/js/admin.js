(async function () {
  ...
  function montarTabelaHorariosSemana() {
    ...
    container.innerHTML = DIAS_SEMANA_NOMES.map((nome, dia) => `
      <div class="linha-horario-semana" style="display:flex; align-items:center; gap:8px; flex-wrap:wrap; padding:6px 0; border-bottom:1px solid var(--borda);">
        ...
        <span class="texto-suave">as</span>
  ...
  function atualizarLinhaHorarioSemana(dia) {
    ...
    ['entrada', 'saida', 'carga'].forEach((campo) => {
      document.getElementById(`hsem-${campo}-${dia}`).disabled = !ativo;
    });
  }
  // Preenche a tabela a partir da config salva (edicao) ou reseta pro estado
  ...
  // Igual ao marcacoesDoDia() do common.js, mas com um botao de excluir em
  // cada marcacao — usado so na tela de admin, pra corrigir/remover
  // lancamentos errados (ex.: gerados por um esquecimento de sequencia).
    ...
    const chips = dia.eventos
      .map(
        ...
        <span class="marca-chip" title="${e.label}">
          ...
          ${e.id ? `<button type="button" class="btn-excluir-marca" data-excluir-marca="${e.id}" title="Excluir esta marcacao">&times;</button>` : ''}
        </span>`
      )
      .join(' ');
    let notas = '';
        ...
        afterRender: () => {
          tbody.querySelectorAll('[data-excluir-marca]').forEach((btn) => {
            btn.addEventListener('click', async (ev) => {
              ev.stopPropagation();
              if (!window.confirm('Excluir esta marcacao? Isso pode alterar as horas calculadas do dia.')) return;
              try {
