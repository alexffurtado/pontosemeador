(async function () {
  ...
  async function carregarStatusHoje() {
    ...
    if (dados.sessaoAbertaDeOutroDia) {
      avisoPlantao.innerHTML = `Voce tem uma marcacao em aberto desde <strong>${dados.sessaoAbertaDesde}</strong> (plantao/sobreaviso que passou da meia-noite). O proximo toque no botao abaixo vai registrar: <strong>${dados.proximoLabel}</strong>.`;
  ...
  document.getElementById('btn-confirmar-correcao').addEventListener('click', async () => {
    ...
    if (!tipo) return;
    if (!window.confirm('Confirma o registro manual deste tipo de marcacao com o horario de agora?')) return;
...
<style>
  ...
  <div class="campo-imp"><span class="rotulo-imp">Descricao:</span></div>
  ...
  <div class="assinatura">
    <div class="linha-assinatura">Assinatura do colaborador</div>
      ...
      if (!janela) {
        window.alert('O navegador bloqueou a janela de impressao. Permita pop-ups para este site e tente novamente.');
        return;
      }
      ...
      // Fallback caso onload nao dispare (alguns navegadores com document.write).
  ...
  document.getElementById('form-justificativa').addEventListener('submit', async (ev) => {
    ...
    try {
      ...
      areaMensagem.innerHTML = '<div class="mensagem-sucesso">Justificativa salva. Voce pode imprimi-la na lista abaixo.</div>';
