...
function tagStatusDia(dia) {
  ...
  if (dia.eventos.length === 0) {
    if (dia.feriado) return '<span class="tag tag-feriado">Feriado</span>';
    return dia.diaUtil ? '<span class="texto-suave">-</span>' : '<span class="tag tag-folga">Folga</span>';
  }
  const tags = [];
  if (dia.feriado) tags.push('<span class="tag tag-feriado">Feriado</span>');
